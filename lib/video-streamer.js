    ...
    streamVideo(videoPath) {
        return new Promise((resolve, reject) => {
            ...
            const ffmpeg = spawn('ffmpeg', [
                ...
                '-c:v', 'libx264',
                '-preset', 'ultrafast',          // Fastest encoding for lowest CPU
                '-tune', 'zerolatency',
                '-profile:v', 'baseline',        // Baseline = least CPU intensive
                '-level', '3.1',                 // Lower level for better compatibility
                '-r', '25',                      // 25fps instead of 30 (20% less CPU)
                '-b:v', '3000k',                 // 3Mbps (still good quality)
                '-maxrate', '3500k',             // Lower maxrate
                '-bufsize', '4000k',             // Smaller buffer = less memory
                '-pix_fmt', 'yuv420p',
                '-g', '100',                     // Larger GOP = fewer keyframes = less CPU
                '-keyint_min', '50',             // Minimum keyframe interval
                '-sc_threshold', '0',            // Disable scene detection (saves CPU)
                '-bf', '0',                      // No B-frames for lower latency/CPU
                '-refs', '1',                    // Single reference frame (less CPU)
                '-c:a', 'aac',
                '-ar', '44100',
