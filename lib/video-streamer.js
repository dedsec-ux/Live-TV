...
class VideoStreamer {
    ...
    streamVideo(videoPath) {
        return new Promise((resolve, reject) => {
            ...
            // Kill timeout - safety timeout for hung processes (very generous)
            killTimeout = setTimeout(() => {
                if (this.currentProcess) {
                    console.warn(`[STREAMER ${this.channelId}] FFmpeg timeout (60min), forcing kill...`);
                    try {
                        this.currentProcess.kill('SIGKILL');
                        this.currentProcess = null;
                    } catch (err) {
            ...
            }, 60 * 60 * 1000); // 60 minutes max per video (for very long videos)
        });
    }
