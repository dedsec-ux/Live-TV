...
class PipeManager {
    ...
    getWriteStream() {
        if (!this.writeStream) {
            ...
            // Add error handling to prevent EPIPE crashes
            this.writeStream.on('error', (error) => {
                if (error.code === 'EPIPE') {
                    console.log(`[PIPE ${this.channelId}] Write stream closed (EPIPE) - reader disconnected`);
                } else {
            ...
            this.writeStream.on('finish', () => {
                console.log(`[PIPE ${this.channelId}] Write stream finished`);
            });
        }
